    ...
    new TreeMirrorClient(document, {
      initialize: (rootId, children) => {
        this.channel.whisper('initialize', {
          rootId,
          children,
          base : window.tempHost ?  window.tempHost : location.href.match(/^(.*\/)[^\/]*$/)[1]
        });
      },
  ...
  protected attachAttributeHandlersToInputs() {
    ...
    document.querySelectorAll('select').forEach((element) => {
      element.onchange = (event) => {
        event.target.setAttribute('selected-option',  event.target.selectedIndex);
      }
    });

    document.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach((element) => {
      element.onchange = (event) => {
        document.querySelectorAll(`input[type="radio"][name="${event.target.name}"]`).forEach((element) => {
