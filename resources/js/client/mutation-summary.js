      ...
      switch (mutation.type) {
        case "childList":
          ...
          for (var i = 0; i < mutation.removedNodes.length; i++) {
            ...
            if (node) this.getChange(node).removedFromParent(mutation.target);
          }
          for (var i = 0; i < mutation.addedNodes.length; i++) {
            var node = mutation.addedNodes[i];
            if (node) this.getChange(node).insertedIntoParent();
          }
          break;
...
MutationProjection.prototype.getOldParentNode = function(node) {
  ...
  if (
    reachabilityChange === 0 /* STAYED_OUT */ ||
    reachabilityChange === 1 /* ENTERED */
  )
...
MutationProjection.prototype.attributeChangedNodes = function(
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (
      2 /* STAYED_IN */ !== this.treeChanges.reachabilityChange(node) ||
      2 /* STAYED_IN */ !== this.matchabilityChange(node)
    ) {
      continue;
    }
  ...
  while (accum !== 2 /* STAYED_IN */ && i < matchChanges.length) {
    switch (matchChanges[i]) {
      case 2 /* STAYED_IN */:
        ...
        break;
      case 1 /* ENTERED */:
        if (accum === 5 /* EXITED */) accum = 2 /* STAYED_IN */;
        else accum = 1 /* ENTERED */;
        break;
      case 5 /* EXITED */:
        if (accum === 1 /* ENTERED */) accum = 2 /* STAYED_IN */;
        else accum = 5 /* EXITED */;
        break;
    }
